...
namespace ECommBackend.DTOs.MapToDomain
{
    public static class UserMapToDomain
    {
        ...
        //}
        public static UserDTO ModelToRecordDTO(this UserModel _user)
        {
            return new UserDTO(
                _user.UserId,
                _user.FirstName,
                _user.LastName,
                _user.Email,
                _user.ProductsBought.Select(x=>x.ModelToRecordDTO()).ToList(),
                _user.CreatedDate,
                _user.Orders.Select(x=> x.ModelToRecordDTO()).ToList()
                );
        }
    }
}
